...
- 物品与背包
  ...
  - `src/ui/inventory-drag-manager.ts` 提供场景级共享拖拽管理器；所有已打开的库存 pane 会在显示时注册到同一套拖拽上下文，关闭时自动注销。
  - 跨容器拖拽已支持“按鼠标落点格子放置”，不再局限于转移到目标容器中的首个空位；玩家背包、箱子面板与测试实验室容器之间可直接互拖。
  - `InventoryUI` 与 `StorageUI` 已统一复用 `src/ui/inventory-pane.ts` 作为单库存面板，避免玩家背包绘制逻辑在多个 UI 中重复维护。
  - `InventoryLabUI` 已改为真正的 `GridContainerComponent + InventoryPane` 组合体，按 `B` 打开，用于验证统一拖拽链路与新容器行为。
...
src/
  ...
  ui/                     # HUD/库存等界面
    ...
    inventory-drag-manager.ts # 场景级共享拖拽管理器，统一所有已打开库存面板的互拖流程
...
- 库存 UI 交互已经改为“pane 注册制”：
  - 任何需要参与拖拽的面板都应复用 `InventoryPane`，并在显示时向 `inventory-drag-manager` 注册 `pane + container + 坐标转换`。
  - 若新增库存 UI 未注册到共享拖拽管理器，即使外观是网格，也不会参与跨界面互拖。
